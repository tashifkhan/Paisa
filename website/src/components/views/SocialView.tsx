    ...
    <div className="flex flex-col h-full bg-(--background) pb-24 overflow-y-auto hide-scrollbar transition-colors duration-300">
      <header className="flex justify-between items-center p-6">
        <div className="flex flex-col">
          ...
          <p className="text-(--muted-foreground) text-sm">Friends & Shared Expenses</p>
        </div>
      ...
      <div className="px-6 mb-6">
        <div className="flex justify-between items-center bg-(--muted) rounded-[2rem] p-1 text-sm font-medium">
          <button
            ...
            className={`flex-1 py-3 rounded-[2rem] transition-all ${socialTab === 'debts' ? 'bg-(--primary) text-(--primary-foreground) shadow-md' : 'text-(--muted-foreground)'}`}
          >
            Friends
          </button>
          <button
            onClick={() => setSocialTab('groups')}
            className={`flex-1 py-3 rounded-[2rem] transition-all ${socialTab === 'groups' ? 'bg-(--primary) text-(--primary-foreground) shadow-md' : 'text-(--muted-foreground)'}`}
          >
            Groups
          </button>
        </div>
      ...
      {socialTab === 'debts' ? (
        <>
          {/* Net Balance Card */}
          <div className="px-6 mb-6">
            <div className={`p-6 rounded-[2rem] shadow-lg relative overflow-hidden ${netBalance >= 0 ? 'bg-(--chart-4)' : 'bg-(--chart-2)'} text-white`}>
          ...
          <div className="px-6 space-y-3">
            {debts.map(debt => (
              <div key={debt.id} className="flex items-center justify-between p-4 bg-(--card) border border-(--border) rounded-3xl hover:bg-(--muted)/50 transition-all cursor-pointer">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-(--foreground)">{debt.name}</h3>
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-(--muted-foreground)">{debt.type === 'owed_to_me' ? 'Credit' : 'Debt'}</p>
                </div>
              </div>
            ))}
            ...
            <button className="w-full py-4 border-2 border-dashed border-(--border) rounded-[2rem] text-(--muted-foreground) font-medium hover:bg-(--muted) transition-colors flex items-center justify-center gap-2 mt-4">
              <div className="w-6 h-6 rounded-full bg-(--primary) text-(--primary-foreground) flex items-center justify-center text-lg leading-none pb-1">+</div>
              Add New Contact
            </button>
          </div>
        </>
        ...
        <div className="px-6 space-y-4">
          {groups.map(group => (
            <div key={group.id} onClick={() => setCurrentView('group-detail')} className="bg-(--card) border border-(--border) rounded-[2rem] p-5 hover:bg-(--muted)/50 transition-all cursor-pointer">
